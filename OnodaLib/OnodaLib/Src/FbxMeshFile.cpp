...
bool FbxMeshFile::LoadModel(const char* modelfile_name, const char* texfile_name)
{
	if (dev == NULL)
...
void FbxMeshFile::RenderE(Vector3 edgeSize, Vector3 edgeColor) {
	DirectGraphics* graphics = DirectGraphics::GetInstance();

	graphics->SetUpContextEdge(edgeSize, edgeColor,pos, scale, degree, axis, axisAngle, axisR, m_VertexShader, m_GeometryShader, m_PixelShader);
	
	DrawVertex();
}
void FbxMeshFile::RenderTV() {
	DirectGraphics* graphics = DirectGraphics::GetInstance();

	graphics->SetUpContextTextureView(pos, scale, degree, axis, axisAngle, axisR, m_VertexShader, m_GeometryShader, m_PixelShader);

	DrawVertex();
}
void FbxMeshFile::RenderCM() {
	DirectGraphics* graphics = DirectGraphics::GetInstance();
	graphics->SetUpContextCubeMap(pos, scale, degree, axis, axisAngle, axisR, m_VertexShader, m_GeometryShader, m_PixelShader);
	DrawVertex();

	graphics->m_Context->GenerateMips(graphics->cubeMapSRView);
}
...
bool FbxMeshFile::LoadFbxFile(const char* modelfile_name,const char* texfile_name)
{
	...
	// sceneにインポート
	fbx_importer->Import(fbx_scene);//めっちゃおもい

	FbxGeometryConverter converter(fbx_manager);
	// アニメーションが読み込めない原因？メッシュに使われているマテリアル単位でメッシュを分割する
	converter.SplitMeshesPerMaterial(fbx_scene, true);//バカおもい
...
void FbxMeshFile::SetMaterialColor(DirectGraphics* graphics, ObjMaterial& material)
{
	...
	constant->MaterialSpecular = DirectX::XMFLOAT4(material.Specular[0], material.Specular[1], material.Specular[2], material.Specular[3]);
...
VertexShader* CreateShader_V(const char* VertexShaderFilePath) {
	...
	vs->Create(dev, VertexShaderFilePath);
	return vs;
}
GeometryShader* CreateShader_G(const char* GeometryShaderFilePath) {
	...
	gs->Create(dev, GeometryShaderFilePath);
	return gs;
}
PixelShader* CreateShader_P(const char* PixelShaderFilePath) {
	...
	ps->Create(dev, PixelShaderFilePath);
	return ps;
}
